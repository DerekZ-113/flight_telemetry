...
public:
    ...
    // explicit: with seed defaulted this is effectively a one-argument
    // constructor, and a bare integer must never silently become a source.
    explicit SimulatedSource(uint64_t interval_ms, uint32_t seed = 42);
